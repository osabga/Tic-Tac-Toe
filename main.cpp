...
int minimax(char positions[], int depth, bool player) {
    ...
    // Empate
    if (TerminaJuego(positions))  return 0;
...
void onePlayer(char positions[]) {
    bool win = false, player = true;
    int plays = 0, lastPosition;

    while (!win && plays < 9) {

        if (player) lastPosition = bestMove(positions);
        else lastPosition = askPosition(positions, player);
...
void playerNumbers() {
    ...
    if (res == "1") onePlayer(positions);
    else if (res == "2") twoPlayers(positions);
    else if (res == "exit") cout << "Thanks for playing!" << endl;
    else {
        cout << "Invalid response" << endl
             << endl;
        playerNumbers();
